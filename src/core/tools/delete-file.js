...
// （删测试目录、清理垃圾文件）后补上，立项理由与三档危险度划分见 design-notes.md D13。
//
// 只删单个文件，不处理目录（目录删除是 rm_empty_dir 的职责，见该文件）。
// 安全铁律（CLAUDE.md 第 5 节）：destructive 操作，写盘前必须过 diff 确认；确认框展示
...
(function () {
  ...
  const definition = {
    ...
    description:
      ...
      '只有 move/write 覆盖，没有纯删除，是官方有意的保守立场，见 design-notes D13）。只能删' +
