...
// 扩展工具（官方无对应；立项理由见 design-notes.md D14）：edit_file 靠内容精确匹配定位
// （D5 的核心安全设计，防止改错地方），擅长"改一小处"，但不擅长"整行/整段重写"——重写时
...
//     报错里的行号一致；正确处理 CRLF。
//
// 行号失效风险（D14 已论证为什么不加校验参数）：调用前本就会先读文件拿到当下行号
...
(function () {
  ...
  const definition = {
    ...
    description:
      ...
      '对应，见 design-notes D14）。start_line 和 end_line 都从 1 开始、都包含在被替换的' +
